...
def restore_model(sess, saver, model_dir, step=None):
    ...
    if step is None:
        ...
        if ckpt is not None and tf.compat.v1.train.checkpoint_exists(ckpt.model_checkpoint_path):
            model_path = ckpt.model_checkpoint_path
        else:
            raise ValueError("model not found in %s" % model_dir)
    else:
        model_path = "%s/fhvae-%s" % (model_dir, step)
        if not tf.compat.v1.train.checkpoint_exists(model_path):
...
def _valid(sess, model, sum_vars, iterator):
    ...
    sum_vals = [0. for _ in range(len(sum_vars))]
    tot_segs = 0.
    for x_val, y_val, n_val, c_val in iterator(bs=2048):
        mu2_val = _make_mu2(mu2_dict, y_val)
        feed_dict = {model.xin: x_val, model.xout: x_val,
                model.n:n_val, model.cReg:c_val, model.mu2:mu2_val}
